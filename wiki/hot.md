...
|---|---|---|---|
| 2026-09-23 | `mesure` | **LES 119 MOISSONNES SONT TRIES** — quatre sessions separees, lots de 30, echelle de `D15` : **10 `oui`, 29 `partiel`, 80 `non`**, soit **39 retenus**. Avec les 17 fiches, le gisement est de **56** pour le lot de 50 qu'exige `D25` : jouable, sans marge | DEUX FAUTES ATTRAPEES PAR LEURS PROPRES GARDES, ET LA SECONDE EST LA PLUS INSTRUCTIVE. (1) `corpus/merge_triage.py` a REFUSE le compte : 119 attendus, 118 tries — le lot 3 avait saute « Russia-Ukraine crisis ». Renvoye a son trieur plutot que tranche par moi. C'est `L21`, et le garde etait ecrit exactement pour ca. (2) LE FICHIER ECRIT PORTAIT LE MEME VERDICT SUR SES 119 LIGNES, une variable de boucle fuitee d'une boucle precedente — et pourtant les comptes IMPRIMES etaient justes, le garde `L21` PASSAIT, et rien n'a leve. Parce que les deux se calculaient sur L'ENTREE. **Un controle en amont d'une transformation ne dit rien de ce qui en sort.** D'ou `L24`, et un garde qui RECOMPTE le fichier produit avant de l'ecrire. Plus dangereux que `L23` du matin meme : celle-la cassait bruyamment, celle-ci produisait un resultat plausible et faux. LES 10 `oui` SONT EXACTEMENT CE QU'ON ESPERAIT Y TROUVER : momentum intrajournalier (global, APAC, S&P500/CSI300, US time-series), volatilite realisee (realized range, realized power variation), effets de calendrier (day of the week), or et petrole. Tous calculables sur des barres OHLCV. RESERVE HONNETE, NON TRANCHEE : le lot 1 a rendu **0 `oui`** quand le lot 3 en a rendu **4**. Les lots sont ordonnes par titre, donc l'ecart peut venir du contenu autant que de la severite du trieur — je ne sais pas departager et je ne le suppose pas. CE TRIAGE N'EST PAS NOTE : aucun etalon humain n'existe pour ces papiers, donc `score_triage.py` ne peut pas le juger. Ce qui fonde la confiance est le passage 1 du 2026-09-20 sur les 20 d'`AMORCE.md` (A/B/C/D = 1/0/2/0), et rien d'autre. `corpus/triage_harvest.py` fabrique l'entree du trieur depuis la base vectorielle, la ou `make_triage_input.py` la tire d'`AMORCE.md` : il n'y a rien a faire fuiter ici puisque aucun verdict n'existe, mais un `join` simple faisait disparaitre SANS ERREUR le papier dont l'extraction du PDF n'avait rien rendu — 118 au lieu de 119, encore `L21`, corrige en `left join lateral`. Aucun IC, aucun test depense : counted_tests **56** |
