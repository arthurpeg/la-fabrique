...
| **Décision la plus récente** | `decisions/DECISION-19-la-version-de-l-extracteur-de-texte.md` — la **version** de `pypdf` fait partie de la définition du texte qui fait foi. Épinglée à |
| **Tests au registre** | 145 |
| **Idées abandonnées recensées** | 49 |
| **Entrées au journal** | 39 |
...
8. **Ficher les 14 papiers restants** — `G1` exige zéro atteignable non fiché.
   `python corpus/extract_fiche.py --list` dit lesquels. **C'est la prochaine
   action**, et c'est le gros du travail restant de la phase 07 : chacun demande
   une session séparée qui n'a pas lu `corpus/fiches/`.
